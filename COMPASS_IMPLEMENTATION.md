...
- `/register`: email OTP verification, age, gender, language, strong password, and a generated sign-in alias with a Shuffle alias button. Email is used transiently for verification, not saved to the operational user record. OTPs expire after 10 minutes, permit three failed attempts, and resend is limited to once per minute per email. Successful verification expires after 10 minutes and is consumed on registration. New accounts persist the actual OTP verification time in `users.email_verified_at` (UTC); historical null timestamps are not fabricated. Multiple seekers can register from one shared IP. OTP actions have separate request limits, and failed mail delivery releases the resend cooldown. Consent is recorded separately at `/seeker/consent` before screening. Existing email accounts continue to sign in.
