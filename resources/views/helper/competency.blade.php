        ...
        <div class="lg:col-span-1">
            <div class="card" style="text-align:center;padding:32px 24px;">
                @if($latest)
                    ...
                    <div style="font-size:64px;font-weight:800;color:var(--green-500);line-height:1.1;margin:8px 0;">{{ number_format($latest->normalized_score, 1) }}<span class="text-lg text-gray-400"> / 5</span></div>
                    ...
                    @foreach($breakdown as $key => $score)
                        <div class="mb-4">
                            <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                                ...
                                <span class="text-sm font-semibold text-gray-800">{{ number_format($score, 1) }} / 5</span>
                            </div>
                            <div class="progress-bar"><span style="width:{{ min(100, max(0, $score * 20)) }}%"></span></div>
                        </div>
                    @endforeach
                </div>
            @endif
                    ...
                    <div style="display:flex;align-items:flex-end;gap:12px;height:160px;padding:8px 4px;">
                        @foreach($trend as $point)
                            <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:100%;">
                                ...
                                <div style="width:100%;max-width:48px;background:linear-gradient(180deg,#38C172,var(--green-500));border-radius:8px 8px 0 0;transition:height .4s ease;height:{{ $point['score'] }}%;" title="{{ $point['label'] }}: {{ $point['score'] }}%"></div>
