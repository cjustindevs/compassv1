...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3>Overall Score: {{ round($evaluation->normalized_score * 20, 1) }}%</h3>
            ...
            @foreach($evaluation->score_breakdown as $key => $score)
                <div>
                    <div style="display:flex;justify-content:space-between;margin-bottom:6px;">
                        ...
                        <span class="text-sm font-semibold text-gray-800">{{ round(($score > 5 ? $score : $score * 20), 1) }}%</span>
                    </div>
                    <div class="progress-bar"><span style="width:{{ min(100, $score > 5 ? $score : $score * 20) }}%"></span></div>
                </div>
            @endforeach
        </div>
        <hr class="divider">
...
<x-supervision-history :record="$evaluation" />
@endsection
