...
class RolePermissionController extends Controller
{
    ...
    private function localPermissionMatrix(): array
    {
        return [
            ...
            $this->category('users', 'Users', 'users', ['read','create']),
            $this->category('reports', 'Reports', 'file-text', ['read']),
            $this->category('announcements', 'Announcements', 'megaphone', ['read']),
            $this->category('system-health', 'System Health', 'activity', ['read']),
            $this->category('audit-logs', 'Audit Logs', 'audit-log', ['read']),
            $this->category('settings', 'Settings', 'sliders', ['read','update']),
            $this->category('referral-access', 'Referral Access', 'inbox', []),
            $this->category('clinical-notes', 'Clinical Notes', 'clipboard', []),
            $this->category('completed-cases', 'Completed Cases', 'folder-check', []),
        ];
    }
